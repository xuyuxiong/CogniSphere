...
const App: React.FC = () => {
  ...
  const [importing, setImporting] = useState(false); // State for URL/File import loading
  const [templates, setTemplates] = useState<PromptTemplate[]>([]);
  const [historyModalOpen, setHistoryModalOpen] = useState(false);

  // Refs
  const fileInputRef = useRef<HTMLInputElement>(null); // For Image inside editor
  const importFileRef = useRef<HTMLInputElement>(null); // For New Note from File
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      // Migration: Ensure 'folder' property exists
  ...
  const handleImportFileNote = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type.startsWith('text') || file.name.endsWith('.md') || file.name.endsWith('.js') || file.name.endsWith('.ts') || file.name.endsWith('.json')) {
       ...
       // Fallback for non-text files, maybe unsupported or treat as base64 attachment in future
  ...
  const handleImportUrlNote = async () => {
    ...
    try {
      ...
      // This is a free public service that handles SPA rendering and cleaning
      ...
      // Basic extraction of title from the markdown if Jina provides it in frontmatter or first line
      // Usually Jina returns: "Title: ... \n\n URL Source: ... \n\n Markdown Content..."
      const titleMatch = text.match(/^Title:\s*(.+)$/m);
      const title = titleMatch ? titleMatch[1] : '导入的网页笔记';
      
      // Clean up Jina's metadata slightly if desired, or keep it for reference
      // Let's keep it as it's useful context
  ...
  const handleSearch = async (query: string) => {
    ...
    if (settings.useSemanticSearch && settings.aiConfig.apiKey && query.length > 3) {
      ...
      try {
        ...
        // In a full app, we would merge/sort these. For now, we log them.
      } catch (e) {
        console.error(e);
      } finally {
        setLoadingAI(false);
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (data.notes && Array.isArray(data.notes)) {
           ...
           if (data.templates) {
             for (const t of data.templates) await StorageService.saveTemplate(t);
           }
  ...
  const renderSidebar = () => (
    <div className="w-64 bg-white dark:bg-dark-card border-r border-gray-200 dark:border-gray-800 flex flex-col h-screen transition-colors duration-200 shrink-0">
  ...
  const renderTreeView = () => {
    ...
    filteredNotes.forEach(note => {
      ...
      if (parts.length > 0) {
        const rootFolder = parts[0]; // Simplified 1-level tree for demo, can be recursive
                       ...
                       <div key={t.id} className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-slate-800/30">
                          <div className="flex gap-2 mb-2">
                            ...
                            <button onClick={async () => {
                               ...
                               setTemplates(list);
                               // In a real app we'd delete from DB too, for now we just won't save it next time or needs a delete method
